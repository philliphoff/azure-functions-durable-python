# A "deterministic" sample of Azure Durable Functions with OpenAI Agents SDK
...
1. Submit a GET request to the Azure Functions endpoint to invoke the agent with input passed as the query parameter `input`
   ```bash
   curl -G "http://localhost:7071/api/agents/myagent" --data-urlencode "input=Write story about the world's first cat-astronaut."
   ```
